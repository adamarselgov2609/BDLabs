# models.py
# SQLAlchemy ORM модели — отражение таблиц БД в классы Python

from sqlalchemy import Column, Integer, BigInteger, String, Text, Numeric, Boolean, Date, DateTime, ForeignKey, CheckConstraint
...
class Booking(Base):
    ...
    listing_id = Column(BigInteger, ForeignKey("listings.listing_id", ondelete="CASCADE"), nullable=False)
    guest_id = Column(BigInteger, ForeignKey("users.user_id", ondelete="CASCADE"), nullable=False)
...
class Review(Base):
    ...
    booking_id = Column(BigInteger, ForeignKey("bookings.booking_id", ondelete="CASCADE"), unique=True, nullable=False)
